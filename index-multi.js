...
function loadWallets() {
  try {
    ...
    addLog(`Gagal membaca file wallet.txt: ${error.message}`, 'system');
    return [];
  }
}
...
function getNextWallet() {
  if (wallets.length === 0) {
    throw new Error('Tidak ada wallet yang tersedia.');
  }
  const wallet = wallets[currentWalletIndex];
  currentWalletIndex = (currentWalletIndex + 1) % wallets.length; // Iterasi melalui wallet
  return wallet;
}
...
async function updateWalletData(wallet) {
  try {
    const provider = new ethers.JsonRpcProvider(RPC_URL);
    const balanceMON = await provider.getBalance(wallet.address);
    const balanceWMON = await new ethers.Contract(
      WMON_ADDRESS,
      ERC20_ABI,
      provider
    ).balanceOf(wallet.address);
    const balanceWETH = await new ethers.Contract(
      WETH_ADDRESS,
      ERC20_ABI,
      provider
    ).balanceOf(wallet.address);
    const balanceUSDC = await new ethers.Contract(
      USDC_ADDRESS,
      ERC20_ABI,
      provider
    ).balanceOf(wallet.address);
    const balanceHEDGE = await new ethers.Contract(
      HEDGE_ADDRESS,
      ERC20_ABI,
      provider
    ).balanceOf(wallet.address);
...
async function executeSwap(
  ...
  addLog(
    ...
    } dengan jumlah ${ethers.formatEther(
      amount
    )} menggunakan wallet ${getShortAddress(wallet.address)}`,
    'rubic'
  );
  ...
  try {
    ...
    await endInitialRubicRequest(txHash, wallet.address, amount, swapToWMON);
    await sendRubicRequest(txHash, wallet.address, swapToWMON);
...
async function runAutoSwap() {
  ...
  promptBox.readInput('Masukkan jumlah swap Rubic:', '', async (err, value) => {
    ...
    for (let i = 1; i <= loopCount; i++) {
      try {
        const walletData = getNextWallet(); // Pilih wallet secara iteratif
        const provider = new ethers.JsonRpcProvider(RPC_URL);
        const wallet = new ethers.Wallet(walletData.privateKey, provider);

        await addTransactionToQueue(async (nonce) => {
          await executeSwap(i, loopCount, wallet, true, nonce);
        }, `Rubic Swap - Iterasi ${i}`);
      } catch (error) {
        addLog(`Rubic: Error pada iterasi ${i}: ${error.message}`, 'rubic');
      }
...
async function runTayaAutoSwapRandom() {
  for (let i = 1; i <= loopCount; i++) {
    try {
      ...
      addLog(`Taya: Error pada iterasi ${i}: ${error.message}`, 'taya');
    }
  }
}
