  ...
  try {
    ...
    await axios.post(`${RUBIC_API_URL}?valid=false`, payload, {
      headers: {
        ...
        Cookie: rubicCookie, // Use the wallet's Rubic cookie here
      },
      proxy: {
        protocol: 'http',
...
async function sendRubicRequest(txHash, walletAddress, rubicCookie, proxy) {
  try {
    const payload = {
      success: true,
      hash: txHash,
...
async function checkRubicRewards(walletAddress, rubicCookie, proxy) {
  try {
...
async function wrapMON(index, total, wallet, proxy) {
  ...
  try {
    ...
    await sendRubicRequest(tx.hash, signer.address, wallet.rubicCookie, proxy);
    await checkRubicRewards(signer.address, wallet.rubicCookie, proxy);
  } catch (error) {
  ...
  while (true) {
    ...
    for (let i = 0; i < wallets.length; i++) {
      ...
      for (let j = 1; j <= parseInt(loopCount); j++) {
        await wrapMON(j, loopCount, wallet, proxy);
      }
    }
