...
const WMON_ADDRESS =
  process.env.WMON_ADDRESS || '0x760AfE86e5de5fa0Ee542fc7B7B713e1c5425701';
const ROUTER_ADDRESS = process.env.ROUTER_ADDRESS || WMON_ADDRESS;
const RUBIC_API_URL =
  process.env.RUBIC_API_URL ||
...
const HEDGE_ADDRESS =
  process.env.HEDGE_ADDRESS || '0x04a9d9D4AEa93F512A4c7b71993915004325ed38';
...
figlet.text(
  'MONAD AUTO SWAP'.toUpperCase(),
  { font: 'Standard', horizontalLayout: 'default' },
  (err, data) => {
    if (err)
      headerBox.setContent('{center}{bold}MONAD AUTO SWAP{/bold}{/center}');
    else
      headerBox.setContent(
        `{center}{bold}{green-fg}${data}{/green-fg}{/bold}{/center}`
      );
    safeRender();
  }
);
const descriptionBox = blessed.box({
  ...
  width: '100%',
  content:
    '{center}{bold}{bright-cyan-fg}=== Telegram Channel 🚀 : NT Exhaust (@NTExhaust) ==={/bright-cyan-fg}{/bold}{/center}',
  tags: true,
...
function stopAllTransactions() {
  if (autoSwapRunning || tayaSwapRunning || hedgemonySwapRunning) {
    ...
    addLog(
      'Stop All Transactions command received. Semua transaksi telah dihentikan.',
      'system'
    );
  }
}
...
function addTransactionToQueue(transactionFunction, description = 'Transaksi') {
  ...
  transactionQueue = transactionQueue.then(async () => {
    ...
    try {
      ...
      if (
        error.message &&
        error.message.toLowerCase().includes('nonce has already been used')
      ) {
        nextNonce++;
        addLog(
        ...
        while (retries < MAX_RPC_RETRIES) {
          try {
            ...
            retries++;
            addLog(
              `RPC error, percobaan retry ${retries}: ${rpcError.message}`,
              'system'
            );
        ...
        if (retries === MAX_RPC_RETRIES) {
          addLog(
            `RPC tetap error setelah ${MAX_RPC_RETRIES} percobaan. Transaksi dilewati.`,
            'system'
          );
        }
      } else {
...
async function updateWalletData() {
  try {
    ...
    const [balanceMON, balanceWMON, balanceWETH, balanceUSDC, rawHedgeBalance] =
      await Promise.all([
        provider.getBalance(wallet.address),
        new ethers.Contract(WMON_ADDRESS, ERC20_ABI, provider).balanceOf(
          wallet.address
        ),
        new ethers.Contract(WETH_ADDRESS, ERC20_ABI, provider).balanceOf(
          wallet.address
        ),
        new ethers.Contract(USDC_ADDRESS, ERC20_ABI, provider).balanceOf(
          wallet.address
        ),
        new ethers.Contract(HEDGE_ADDRESS, ERC20_ABI, provider).balanceOf(
          wallet.address
        ),
      ]);
  ...
  return Promise.race([
    ...
    new Promise((resolve) => {
      const interval = setInterval(() => {
        if (type === 'rubic' && autoSwapCancelled) {
          clearInterval(interval);
          resolve();
        }
        if (type === 'taya' && tayaSwapCancelled) {
          clearInterval(interval);
          resolve();
        }
        if (type === 'hedgemony' && hedgemonySwapCancelled) {
          clearInterval(interval);
          resolve();
        }
      }, 100);
...
async function endInitialRubicRequest(
  txHash,
  walletAddress,
  amount,
  swapToWMON
  ...
  try {
    ...
    addLog(
      `Rubic: Transaksi terkirim!! Tx Hash: ${getShortHash(txHash)}`,
      'rubic'
    );
  } catch (error) {
    addLog(
      `Rubic: Error in initial Rubic API request: ${error.message}`,
      'rubic'
    );
  }
}
...
async function executeSwap(
  ...
  addLog(
    `Rubic: Memulai swap ${
      swapToWMON ? 'MON ->> WMON' : 'WMON ->> MON'
    } dengan jumlah ${ethers.formatEther(amount)}`,
    'rubic'
  );
  try {
    const tx = swapToWMON
...
async function sendRubicRequest(txHash, walletAddress, swapToWMON) {
  try {
    ...
    addLog(
      `Rubic: Swap ${
        swapToWMON ? 'MON ke WMON' : 'WMON ke MON'
      } selesai!! Tx Hash: ${getShortHash(txHash)}`,
      'rubic'
    );
...
async function runAutoSwap() {
  ...
  promptBox.readInput('Masukkan jumlah swap Rubic:', '', async (err, value) => {
    ...
    addLog(
      `Rubic: Anda memasukkan ${loopCount} kali auto swap Rubic.`,
      'rubic'
    );
    if (autoSwapRunning) {
      addLog(
        'Rubic: Transaksi sudah berjalan. Silahkan stop transaksi terlebih dahulu.',
        'rubic'
      );
      return;
    }
    ...
    for (let i = 1; i <= loopCount; i++) {
      ...
      if (i < loopCount) {
        ...
        addLog(
          `Rubic: Menunggu ${minutes} menit ${seconds} detik sebelum transaksi berikutnya...`,
          'rubic'
        );
...
async function executeTayaSwapRouteWithAmount(
  index,
  total,
  wallet,
  path,
  ...
  if (inputIsETH && path[0].toLowerCase() !== expectedWETH.toLowerCase()) {
    addLog(
      `Taya: Error - Path harus diawali dengan alamat WETH: ${expectedWETH}`,
      'taya'
    );
    return;
  }
  ...
  addLog(
    `Taya: Memulai Swap dengan jumlah: ${ethers.formatEther(amountIn)}`,
    'taya'
  );
  try {
...
async function executeWrapMonToWMON(index, total, wallet, amountInOverride) {
  ...
  addLog(
    `Taya: Melakukan Swap MON ->> WMON dengan jumlah: ${ethers.formatEther(
      amount
    )}`,
    'taya'
  );
  try {
    ...
    addLog(
      `Taya: Error pada wrap transaksi ${index}: ${error.message}`,
      'taya'
    );
  }
}
async function executeUnwrapWMONToMON(index, total, wallet, amountInOverride) {
  ...
  const amount = amountInOverride;
  addLog(
    `Taya: Melakukan Swap WMON ->> MON dengan jumlah: ${ethers.formatEther(
      amount
    )}`,
    'taya'
  );
  try {
    ...
    addLog(
      `Taya: Error pada unwrap transaksi ${index}: ${error.message}`,
      'taya'
    );
  }
}
...
async function runTayaAutoSwapRandom() {
  promptBox.setFront();
  promptBox.readInput(
    'Masukkan jumlah swap Taya (Random Token):',
    '',
    async (err, value) => {
      promptBox.hide();
      screen.render();
      ...
      addLog(
        `Taya: Anda memasukkan ${loopCount} kali auto swap Taya (Random Token).`,
        'taya'
      );
      if (tayaSwapRunning) {
        addLog(
          'Taya: Transaksi sudah berjalan. Silahkan stop transaksi terlebih dahulu.',
          'taya'
        );
        return;
      }
      ...
      for (let i = 1; i <= loopCount; i++) {
        if (tayaSwapCancelled) {
          addLog(
            `Taya: Auto swap (Random Token) dihentikan pada iterasi ${i}.`,
            'taya'
          );
          break;
        }
        ...
        addLog(
          `Taya: Melakukan swap MON ->> ${getTokenSymbol(randomToken)}`,
          'taya'
        );
        const path = [WMON_ADDRESS, randomToken];
        const amountIn = getRandomAmountTaya();
        addLog(
          `Taya: Menggunakan jumlah: ${ethers.formatEther(amountIn)}`,
          'taya'
        );
        await addTransactionToQueue(async (nonce) => {
          await executeTayaSwapRouteWithAmount(
            i,
            loopCount,
            globalWallet,
            path,
        ...
        if (i < loopCount) {
          ...
          const seconds = Math.floor((delay % 60000) / 1000);
          addLog(
            `Taya: Menunggu ${minutes} menit ${seconds} detik sebelum transaksi berikutnya...`,
            'taya'
          );
          await waitWithCancel(delay, 'taya');
          if (tayaSwapCancelled) {
            addLog(
              'Taya: Auto swap (Random Token) dihentikan saat waktu tunggu.',
              'taya'
            );
            break;
          }
...
async function runTayaWrapCycle() {
  promptBox.setFront();
  promptBox.readInput(
    'Masukkan jumlah swap Taya (MON & WMON):',
    '',
    async (err, value) => {
      promptBox.hide();
      screen.render();
      ...
      addLog(
        `Taya: Anda memasukkan ${loopCount} cycle untuk swap Taya (MON & WMON).`,
        'taya'
      );
      if (tayaSwapRunning) {
        addLog(
      ...
      for (let i = 1; i <= loopCount; i++) {
        ...
        if (operation === 'wrap') {
          if (monBalance < amountIn) {
            if (wmonBalance >= amountIn) {
              operation = 'unwrap';
              addLog(
                'Taya: Saldo MON tidak mencukupi, fallback ke unwrap.',
                'taya'
              );
            } else {
              addLog(
                `Taya: Cycle ${i}: Saldo MON dan WMON tidak mencukupi.`,
                'taya'
              );
              continue;
            }
          ...
          if (wmonBalance < amountIn) {
            if (monBalance >= amountIn) {
              operation = 'wrap';
              addLog(
                'Taya: Saldo WMON tidak mencukupi, fallback ke wrap.',
                'taya'
              );
            } else {
              addLog(
                `Taya: Cycle ${i}: Saldo WMON dan MON tidak mencukupi.`,
                'taya'
              );
              continue;
            }
        ...
        if (operation === 'wrap') {
          await addTransactionToQueue(async (nonce) => {
            ...
            await tx.wait();
            addLog(
              `Taya: Tx confirmed!! Tx Hash: ${getShortHash(tx.hash)}`,
              'taya'
            );
            await updateWalletData();
          }, `Taya Wrap (Cycle ${i})`);
          ...
          await addTransactionToQueue(async (nonce) => {
            ...
            const data = router.interface.encodeFunctionData('withdraw', [
              amountIn,
            ]);
        ...
        if (i < loopCount) {
          ...
          const seconds = Math.floor((delay % 60000) / 1000);
          addLog(
            `Taya: Menunggu ${minutes} menit ${seconds} detik sebelum cycle berikutnya...`,
            'taya'
          );
          await waitWithCancel(delay, 'taya');
          if (tayaSwapCancelled) {
...
async function sendTradeHistoryWithRetry(
  txHash,
  wallet,
  amountIn,
  swapToWMON = true,
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const response = await axios.post(
        'https://alpha-api.hedgemony.xyz/trade-history',
        tradePayload,
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      addLog(
        `Hedgemony: Gagal mengirim trade history (attempt ${attempt}): ${error.message}`,
        'hedgemony'
      );
      if (attempt < retries) {
        ...
        addLog(
          'Hedgemony: Semua percobaan retry trade history gagal.',
          'hedgemony'
        );
      }
    }
...
async function sendHedgeTradeHistoryWithRetry(
  txHash,
  wallet,
  amountValue,
  swapToHEDGE,
  ...
  const amountStr =
    typeof amountValue === 'string' ? amountValue : amountValue.toString();
  let buyAmount;
  if (swapToHEDGE) {
    buyAmount =
      (BigInt(amountStr) * MON_TO_HEDGE_CONVERSION_FACTOR) / WEI_PER_ETHER;
  } else {
    buyAmount =
      (BigInt(amountStr) * HEDGE_TO_MON_CONVERSION_FACTOR) / WEI_PER_ETHER;
  }
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      addLog(
        `Hedge Swap: Gagal mengirim trade history (attempt ${attempt}): ${error.message}`,
        'hedgemony'
      );
      if (attempt < retries) {
        await new Promise((resolve) => setTimeout(resolve, delayMs));
        ...
        addLog(
          'Hedge Swap: Semua percobaan retry trade history gagal.',
          'hedgemony'
        );
      }
    }
...
async function runHedgeSwap() {
  promptBox.setFront();
  promptBox.readInput(
    'Masukkan jumlah cycle swap Mon & HEDGE:',
    '',
    async (err, value) => {
      promptBox.hide();
      screen.render();
      ...
      if (isNaN(loopCount) || loopCount <= 0) {
        addLog(
          'Hedge Swap: Input tidak valid. Harus berupa angka positif.',
          'hedgemony'
        );
        return;
      }
      if (hedgemonySwapRunning) {
        addLog(
          'Hedge Swap: Transaksi sudah berjalan. Silahkan stop transaksi terlebih dahulu.',
          'hedgemony'
        );
        return;
      }
      ...
      screen.render();
      addLog(
        `Hedge Swap: Mulai auto swap sebanyak ${loopCount} cycle.`,
        'hedgemony'
      );
      ...
      for (let i = 1; i <= loopCount; i++) {
        if (hedgemonySwapCancelled) {
          addLog(
            `Hedge Swap: Auto swap dihentikan pada cycle ke-${i}.`,
            'hedgemony'
          );
          break;
        }
        ...
        if (swapToHEDGE) {
          ...
          addLog(
            `Hedge Swap: Cycle ${i}: Akan swap MON -> HEDGE sebesar ${ethers.formatEther(
              amountBN
            )} MON`,
            'hedgemony'
          );
          ...
          addLog(
            `Hedge Swap: Cycle ${i}: Akan swap HEDGE -> MON sebesar ${ethers.formatUnits(
              amountBN,
              18
            )} HEDGE`,
            'hedgemony'
          );
          const hedgeContract = new ethers.Contract(
            HEDGE_ADDRESS,
            ERC20_ABI_APPROVE,
            globalWallet
          );
          const hedgeBalance = await hedgeContract.balanceOf(
            globalWallet.address
          );
          if (hedgeBalance < amountBN) {
            addLog(
              `Hedge Swap: Saldo HEDGE tidak cukup. Skip cycle ${i}.`,
              'hedgemony'
            );
            continue;
          }
          const currentAllowance = await hedgeContract.allowance(
            globalWallet.address,
            HEDGEMONY_SWAP_CONTRACT
          );
          if (currentAllowance < amountBN) {
            addLog(
              'Hedge Swap: Allowance HEDGE tidak mencukupi, melakukan approve...',
              'hedgemony'
            );
            const approveTx = await hedgeContract.approve(
              HEDGEMONY_SWAP_CONTRACT,
              ethers.MaxUint256
            );
            addLog(
              `Hedge Swap: Approval tx dikirim: ${getShortHash(
                approveTx.hash
              )}`,
              'hedgemony'
            );
        ...
        if (swapToHEDGE) {
          ...
          payload = {
            chainId: 10143,
            inputTokens: [{ address: HEDGE_ADDRESS, amount: amountStr }],
            outputTokens: [
              {
                address: '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee',
                percent: 100,
        ...
        try {
          const apiResponse = await axios.post(
            'https://alpha-api.hedgemony.xyz/swap',
            payload,
            {
              headers: {
                'Content-Type': 'application/json',
          ...
          if (!multicallTx || !multicallTx.to || !multicallTx.data) {
            ...
            await addTransactionToQueue(async (nonce) => {
              ...
              addLog(
                `Hedge Swap: Tx sent!! Tx Hash: ${getShortHash(tx.hash)}`,
                'hedgemony'
              );
              await tx.wait();
              addLog(
                `Hedge Swap: Tx confirmed!! Tx Hash: ${getShortHash(tx.hash)}`,
                'hedgemony'
              );
              await updateWalletData();
              await sendHedgeTradeHistoryWithRetry(
                tx.hash,
                globalWallet,
                amountStr,
                swapToHEDGE
          ...
          if (error.response && error.response.data) {
            addLog(
              `Hedge Swap: Error: ${JSON.stringify(error.response.data)}`,
              'hedgemony'
            );
          } else {
        ...
        if (i < loopCount) {
          ...
          const seconds = Math.floor((delay % 60000) / 1000);
          addLog(
            `Hedge Swap: Menunggu ${minutes} menit ${seconds} detik sebelum cycle berikutnya...`,
            'hedgemony'
          );
          await waitWithCancel(delay, 'hedgemony');
          if (hedgemonySwapCancelled) {
            addLog(
              'Hedge Swap: Auto swap dihentikan saat waktu tunggu.',
              'hedgemony'
            );
            break;
          }
...
async function runHedgemonySwap() {
  promptBox.setFront();
  promptBox.readInput(
    'Masukkan jumlah swap Hedgemony :',
    '',
    async (err, value) => {
      promptBox.hide();
      screen.render();
      ...
      if (isNaN(loopCount) || loopCount <= 0) {
        addLog(
          'Hedgemony: Input tidak valid. Harus berupa angka positif.',
          'hedgemony'
        );
        return;
      }
      if (hedgemonySwapRunning) {
        addLog(
          'Hedgemony: Transaksi sudah berjalan. Silahkan stop transaksi terlebih dahulu.',
          'hedgemony'
        );
        return;
      }
      ...
      screen.render();
      addLog(
        `Hedgemony: Mulai auto swap sebanyak ${loopCount} kali.`,
        'hedgemony'
      );
      ...
      for (let i = 1; i <= loopCount; i++) {
        if (hedgemonySwapCancelled) {
          addLog(
            `Hedgemony: Auto swap dihentikan pada iterasi ${i}.`,
            'hedgemony'
          );
          break;
        }
        ...
        const amountStr = amountBN.toString();
        if (!swapToWMON) {
          const wmonBalance = await wmonContract.balanceOf(
            globalWallet.address
          );
          addLog(
            `Hedgemony: Akan swap WMON ->> MON sebesar ${ethers.formatEther(
              amountBN
            )}`,
            'hedgemony'
          );
          if (wmonBalance < amountBN) {
            addLog(
              `Hedgemony: Saldo WMON tidak cukup. Skip iterasi ${i}.`,
              'hedgemony'
            );
            continue;
          }
          const currentAllowance = await wmonContract.allowance(
            globalWallet.address,
            HEDGEMONY_SWAP_CONTRACT
          );
          if (currentAllowance < amountBN) {
            addLog(
              'Hedgemony: Allowance WMON tidak mencukupi, melakukan approve...',
              'hedgemony'
            );
            ...
            addLog(
              `Hedgemony: Approval tx dikirim: ${getShortHash(approveTx.hash)}`,
              'hedgemony'
            );
            await approveTx.wait();
          ...
          addLog(
            `Hedgemony: Akan swap MON ->> WMON sebesar ${ethers.formatEther(
              amountBN
            )}`,
            'hedgemony'
          );
        ...
        if (swapToWMON) {
          payload = {
            ...
            outputTokens: [{ address: WMON_ADDRESS, percent: 100 }],
            recipient: globalWallet.address,
            slippage: 0.5,
          };
        } else {
          payload = {
            chainId: 10143,
            inputTokens: [{ address: WMON_ADDRESS, amount: amountStr }],
            outputTokens: [
              {
                address: '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee',
                percent: 100,
        ...
        try {
          ...
          if (!multicallTx || !multicallTx.to || !multicallTx.data) {
            ...
            await addTransactionToQueue(async (nonce) => {
              ...
              addLog(
                `Hedgemony: Tx sent!! Tx Hash: ${getShortHash(tx.hash)}`,
                'hedgemony'
              );
              await tx.wait();
              addLog(
                `Hedgemony: Tx confirmed!! Tx Hash: ${getShortHash(tx.hash)}`,
                'hedgemony'
              );
              await sendTradeHistoryWithRetry(
                tx.hash,
                globalWallet,
                amountStr,
                swapToWMON
          ...
          if (error.response && error.response.data) {
            addLog(
              `Hedgemony: Error: ${JSON.stringify(error.response.data)}`,
              'hedgemony'
            );
          } else {
        ...
        if (i < loopCount) {
          ...
          const seconds = Math.floor((delay % 60000) / 1000);
          addLog(
            `Hedgemony: Menunggu ${minutes} menit ${seconds} detik sebelum transaksi berikutnya...`,
            'hedgemony'
          );
          await waitWithCancel(delay, 'hedgemony');
          if (hedgemonySwapCancelled) {
            addLog(
              'Hedgemony: Auto swap dihentikan saat waktu tunggu.',
              'hedgemony'
            );
            break;
          }
